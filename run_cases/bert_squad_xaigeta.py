"""
BERT SQuAD XAI-GETA Experiment
==============================

Train BERT on SQuAD using the XAI-GETA optimizer with attribution-based importance scoring.

Usage:
    python bert_squad_xaigeta.py --method saliency --weight 0.3 --epochs 3
    python bert_squad_xaigeta.py --method integrated_gradients --weight 0.5 --epochs 1
...
- Bit width range: [b_l, b_u] = [4, 8]
- Exponential: t = 1
- Sparsity: 0.1
- Total Epochs: 3
- Projection Periods B: 4
- Projection Steps K_b: 1 epoch
- Pruning Periods P: 6
- Pruning Steps K_p: 6 epochs
- Optimizer: Adam (3e-5)
"""
...
def train_xai_geta(
    ...
    """
    Train BERT with XAI-GETA optimizer.
    ...
    # NOTE: Skip initial attribution computation for BERT due to input complexity
    # The XAI-GETA optimizer will compute attributions during training
    print("Skipping initial attribution computation for BERT (will compute during training)...")
    ...
    # Training loop
    print(f"\nTraining BERT with XAI-GETA for {config.epochs} epochs...")
...
def main():
    ...
    # Parse arguments
    parser = get_xai_parser('BERT SQuAD XAI-GETA Experiment')
    parser.add_argument(
        '--model-name',
        type=str,
    ...
    # Print configuration
    print("\n" + "=" * 70)
    print("BERT SQuAD XAI-GETA Experiment")
    print("=" * 70)
    ...
    results, trained_model, oto = train_xai_geta(
        trainloader, testloader, trainset, model, oto, tokenizer, config
    )
    ...
    exp_name = generate_experiment_name('xai_geta', config, model_name='bert_squad')
