...
def _is_caffe2_aten_fallback():
    """Compatibility wrapper for is_caffe2_aten_fallback.
    
    In PyTorch 2.x, Caffe2 support was removed, so this always returns False.
    For older versions, we use the symbolic_helper function if available.
    """
    if Version(torch.__version__) >= Version("2.0.0"):
        return False
    return getattr(symbolic_helper, 'is_caffe2_aten_fallback', lambda: False)()
...
def _optimize_trace_graph_no_onnx_operator(
    ...
    # API changed in PyTorch 2.x - the third argument was removed
    if Version(torch.__version__) >= Version("2.0.0"):
